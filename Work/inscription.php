...
<body>
    ...
    if(isset($_POST['submit'])) {
        ...
        $message = [];
        // reccuperation de la photo de profile
        $pos = strpos($_FILES["pp"]["name"], ".");
        $ext = substr($_FILES["pp"]["name"], $pos);
        move_uploaded_file($_FILES["pp"]["tmp_name"], "images/pp/".$name.$ext);
        $imageuser = $name.$ext;
        ...
        $req =    "INSERT INTO users (id, username, password, mail, pp) 
                  VALUES(NULL, '$name', '$pwd', '$mail', '$imageuser')";
